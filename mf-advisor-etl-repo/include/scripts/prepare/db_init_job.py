from include.scripts.utils import _get_db_connection, create_table_for_processed_data, create_table_for_raw_data
import sys
from pyspark.sql import SparkSession
from pyspark.sql.functions import lit
import traceback


def main():
    if len(sys.argv) < 3:
        raise ValueError(
            "Usage: spark-submit read.py <input_path> <current_date>"
        )

    input_path = sys.argv[1]
    current_date = sys.argv[2]

    print(f"✅ Reading dataset from: {input_path}")
    print(f"✅ Current Date processed dataset: {current_date}")

    spark = None
    conn = None
    cursor = None
    try:
        # Start Spark session
        spark = SparkSession.builder.appName("DataBase_Initializer_job").getOrCreate()

        # Read raw data
        df = spark.read.csv(input_path, header=True, inferSchema=True)

        # Get DB connection and initialize schema
        # Get DB connection and hook
        db_conn, pg_hook = _get_db_connection()

        cursor = db_conn.cursor()
        create_table_for_raw_data(cursor)
        create_table_for_processed_data(cursor)
        db_conn.commit()

        print("✅ Table created or already exists in PostgreSQL.")

    except Exception as e:
        print("Error occurred during database initialization job")
        traceback.print_exc()

        if db_conn:
            db_conn.rollback()
            print("Rolled back DB transaction")

        # re-raise so Airflow/Spark marks task as failed
        raise RuntimeError(f"Database initialization job failed: {str(e)}") from e

    finally:
        if cursor:
            cursor.close()
            print("Closed Postgres cursor")
        if db_conn:
            db_conn.close()
            print("Closed Postgres connection")
        if spark:
            spark.stop()
            print("Stopped Spark session")


if __name__ == "__main__":
    main()
